...
static f32 res;
static u64 state0, state1, state2, state3;

u64 splitmix64()
{
	u64 result = (state0 += 0x9E3779B97f4A7C15);
...
u64 xoshiro256()
{
	const u64 result = state0 + state3;
	const u64 t = state1 << 17;
	...
	state3 = (state3 << 45) | (state3 >> 19);

	return result;
}

static inline u64 xoshiro256_res(void)
{
...
{
	state0 = splitmix64(initial_seed);
	state1 = splitmix64(state0);
	state2 = splitmix64(state1);
	state3 = splitmix64(state2);
}
